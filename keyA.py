# coding: utf-8
import numpy as np
import pickle

_diff_target = 0
pears = [['d305a55b2f86142ac6008236696a1a55',
          '0205a55b2f861459c600013669e81a55'],
         ['2819ebbf80d3530452da3c356ea75f98',
          '8f19ebbf80d353b552da02356ea25f98'],
         ['8828f160a6b3da79bb52ca47cc10d809',
          'f228f160a6b3da36bb524c47cc22d809']]

Sbox_inv = np.array([
    
])  # S-box_inv #Fig14


def split(P, j, Nk):  # 4byteごとに分割
    s1 = [int("0x" + P[i: i+j], 0) for i in range(0, len(P), j)]
    S1 = []
    for i in range(Nk):
        S1.append(s1[4*i:4*i+4])
    return S1


def create_key(i, j, mode):
    key = [[0]*4 for x in range(4)]
    if mode == 0:
        key[0][0] = i
        key[3][1] = j
    elif mode == 1:
        key[2][2] = i
        key[1][3] = j
    return key

def create_key2(A, B):
    key = [[0]*4 for x in range(4)]
    key[0][0] = A[0]
    key[3][1] = A[1]
    key[2][2] = B[0]
    key[1][3] = B[1]
    return key

def inv(A, key):
    output = []
    step1 = np.bitwise_xor(A, key)
    step2 = [[step1[0][0], step1[3][1], step1[2][2], step1[1][3]],
             [step1[1][0], step1[0][1], step1[3][2], step1[2][3]],
             [step1[2][0], step1[1][1], step1[0][2], step1[3][3]],
             [step1[3][0], step1[2][1], step1[1][2], step1[0][3]]]
    for x in step2:
        output.append(Sbox_inv[x])
    return output


def cal_diff(A, B, key, mode):
    diff = np.bitwise_xor(inv(A, key), inv(B, key))
    if mode == None:
        return diff[_diff_target]
    else:
        return diff[_diff_target][mode*2:mode*2+2]


with open('D10A.pkl', 'rb') as f:
    origin_D10 = pickle.load(f)


if __name__ == '__main__':
    guess_key = []
    for pear in pears:
        print(pear)
        cand = []
        mode0_lis = []
        mode1_lis = []
        suc = split(pear[0], 2, 4)
        fau = split(pear[1], 2, 4)
        for mode in range(2):
            D10_mode = origin_D10[:,2*mode:2*mode+2].tolist()
            for i in range(256):
                for j in range(256):
                    diff = cal_diff(suc, fau, create_key(i, j, mode),mode).tolist()
                    if diff in D10_mode:
                        if mode == 0:
                            mode0_lis.append([i,j])
                        if mode == 1:
                            mode1_lis.append([i,j])
        D10 = origin_D10.tolist()
        for A in mode0_lis:
            for B in mode1_lis:
                key = create_key2(A, B)
                diff = cal_diff(suc, fau, key, None).tolist()
                if diff in D10:
                    cand.append([key[0][0],key[1][3],key[2][2],key[3][1]])

        print(len(cand))
        guess_key.append(cand)

    with open('keyA.pkl', 'wb') as f:
        pickle.dump(guess_key, f)

