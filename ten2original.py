sbox = []

Rcon = [0x36000000, 0x1b000000, 0x80000000, 0x40000000, 0x20000000,
        0x10000000, 0x08000000, 0x04000000, 0x02000000, 0x01000000]

key = [0]*44

with open('10Rkey.txt', 'r') as f:
    key_10R = f.read()
    print(key_10R)


def split_n(text, n):
    return [text[i*n:i*n+n] for i in range(int(len(text)/n))]


num = 0
for i in split_n(key_10R[2:], 8):
    key[40+num] = int('0x' + str(i), 0)
    num += 1



def subword(A):
    B = '0x'
    for i in range(4):
        B += str(format(sbox[int('0x' +
                                 str(format(rotword, '08x'))[2*i:2*i+2], 0)], '02x'))
    return(int(B, 0))


n = 0
for i in range(40):
    if((i+1) % 4 == 0):
        tmp = '0x' + str(format(key[42-i], '08x'))
        rotword = int('0x' + tmp[4:] + tmp[2:4], 0)
        sub = subword(rotword)
        rcon = Rcon[n] ^ sub
        n += 1
        key[39-i] = rcon ^ key[43-i]
    else:
        key[39-i] = key[43-i] ^ key[42-i]
num = 0


with open('key.txt', mode='w') as f:
    original_key = '0x'
    for i in key[0:4]:
        original_key += format(i, '08x')
    f.write(original_key)

print(original_key)

